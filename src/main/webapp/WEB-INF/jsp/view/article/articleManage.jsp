...
<div class="container" style="width: 90%;">
    ...
    <c:if test="${sessionScope.identity_id ne 4}">
        ...
        <div class="form table">
            <div>
                <form class="layui-form form-inline" action="<%=basePath%>/PageContextEntity/findByPage" role="form" method="post">
    ...
    <c:if test="${sessionScope.identity_id ne 4}">
        <div class="layui-inline">
            ...
            <a href="<%=basePath%>/PageContextEntity/toPageContextEntityWrite"><button type="button" id="addBtn" class="layui-btn layui-btn">添加文章</button></a>
        </div>
    </c:if>
    <br/>
    <br/>
    <div class="table-responsive">
        <table class="table table-striped table-hover" id="countTable">
            ...
            <c:forEach items="${requestScope.page.beanList}" var="PageContextEntity">
                <tr>
                    <td><input name="pagecontextid" value="${PageContextEntity.pagecontextid}" type="checkbox"/></td>
                    <td>${PageContextEntity.title}</td>
                    <td>${fn:substring(PageContextEntity.context,1,15)}</td>
                    <td>${PageContextEntity.author}</td>
                    <td>${PageContextEntity.source}</td>
                    <c:if test="${sessionScope.identity_id ne 4}">
                        <td>
                            <c:if test="${PageContextEntity.verify eq 0}">
                                <label style="color: #FF5722">未审核</label>
                            </c:if>
                            <c:if test="${PageContextEntity.verify eq 1}">
                                已审核
                            </c:if>
                        </td>
                        <td>
                            <c:if test="${PageContextEntity.publish eq 0}">
                                未发布
                            </c:if>
                            <c:if test="${PageContextEntity.publish eq 1}">
                                已发布
                            </c:if>
                        </td>
                        <td>
                            <c:if test="${PageContextEntity.status eq 0}">
                                存在
                            </c:if>
                            <c:if test="${PageContextEntity.status eq 1}">
                                <label style="color: #FF5722">已删除</label>
                            </c:if>
                        </td>
                        <td>
                            <a href="<%=basePath%>/PageContextEntity/toPageContextEntityView?pagecontextid=${PageContextEntity.pagecontextid}"><button type="button" class="layui-btn layui-btn-sm">查看</button></a>
                            <a href="<%=basePath%>/PageContextEntity/toEditPage?pagecontextid=${PageContextEntity.pagecontextid}"><button type="button" class="layui-btn layui-btn-sm layui-btn-normal">编辑</button></a>
                            <button type="button" onclick="return clean(${PageContextEntity.pagecontextid});" class="layui-btn layui-btn-sm layui-btn-danger">删除</button>
                        </td>
                    </c:if>
                    <c:if test="${sessionScope.identity_id eq 4}">
                        <td>
                            <a href="<%=basePath%>/PageContextEntity/toPageContextEntityView?pagecontextid=${PageContextEntity.pagecontextid}"><button type="button" class="layui-btn layui-btn-sm">查看文章</button></a>
                        </td>
                    </c:if>
                </tr>
            </c:forEach>
    ...
    <form class="listForm" name="listForm" method="post" action="<%=basePath%>/PageContextEntity/findByPage">
        <div class="row">
            <div class="form-inline" style="margin-left: 15px;">
                ...
                <ul class="pagination" style="float:right;">
                    <li>
                        <a href="<%=basePath%>/PageContextEntity/findByPage?pageCode=1"><strong>首页</strong></a>
                    </li>
                    <li>
                        <c:if test="${requestScope.page.pageCode > 2}">
                            <a href="<%=basePath%>/PageContextEntity/findByPage?pageCode=${requestScope.page.pageCode - 1}">&laquo;</a>
                        </c:if>
                    </li>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        ...
                        <c:if test="${i != requestScope.page.pageCode}">
                            <li>
                                <a href="<%=basePath%>/PageContextEntity/findByPage?pageCode=${i}">${i}</a>
                            </li>
                        </c:if>
                    </c:forEach>
                    ...
                    <li>
                        <c:if test="${requestScope.page.pageCode < requestScope.page.totalPage}">
                            <a href="<%=basePath%>/PageContextEntity/findByPage?pageCode=${requestScope.page.pageCode + 1}">&raquo;</a>
                        </c:if>
                    </li>
                    <li>
                        <a href="<%=basePath%>/PageContextEntity/findByPage?pageCode=${requestScope.page.totalPage}"><strong>末页</strong></a>
                    </li>
                </ul>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function clean(pagecontextid){
        layer.open({
            ...
            btn1: function(index,layero){
                $.ajax({
                    url: '<%=basePath%>/PageContextEntity/clean.do',
                    type: 'POST',
    ...
    //编辑
    function edit(pagecontextid){
        $.ajax({
            url: '<%=basePath%>/PageContextEntity/toEditPage.do',
            type: 'GET',
            data: {pagecontextid: pagecontextid},
            success: function(data){
                $("body").html(data);
