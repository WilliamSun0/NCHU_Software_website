...
<div class="container" style="width: 95%;">
        ...
        <table class="table table-striped table-hover" id="countTable">
            ...
            <c:forEach items="${requestScope.page.beanList}" var="">
                <tr>
                    ...
                    <td>
                        <button type="button" onclick="return restore(${PageContextEntity.pagecontextid})" class="layui-btn layui-btn-sm layui-btn-normal">恢复</button>
                        <button type="button" onclick="return deletes(${PageContextEntity.pagexontextid});" class="layui-btn layui-btn-sm layui-btn-danger">删除</button>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
    ...
    <form class="listForm" name="listForm" method="post" action="<%=basePath%>/PageContextEntity/findByPage.do">
        <div class="row">
            <div class="form-inline" style="margin-left: 15px;">
                ...
                <ul class="pagination" style="float:right;">
                    <li>
                        <a href="<%=basePath%>/PageContextEntity/findByPage.do?pageCode=1"><strong>首页</strong></a>
                    </li>
                    <li>
                        <c:if test="${requestScope.page.pageCode > 2}">
                            <a href="<%=basePath%>/PageContextEntity/findByPage.do?pageCode=${requestScope.page.pageCode - 1}">&laquo;</a>
                        </c:if>
                    </li>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        ...
                        <c:if test="${i != requestScope.page.pageCode}">
                            <li>
                                <a href="<%=basePath%>/PageContextEntity/findByPage.do?pageCode=${i}">${i}</a>
                            </li>
                        </c:if>
                    </c:forEach>
                    ...
                    <li>
                        <c:if test="${requestScope.page.pageCode < requestScope.page.totalPage}">
                            <a href="<%=basePath%>/PageContextEntity/findByPage.do?pageCode=${requestScope.page.pageCode + 1}">&raquo;</a>
                        </c:if>
                    </li>
                    <li>
                        <a href="<%=basePath%>/PageContextEntity/findByPage.do?pageCode=${requestScope.page.totalPage}"><strong>末页</strong></a>
                    </li>
                </ul>
            </div>
        </div>
...
<script type="text/javascript">
    //彻底删除
    function deletes(pagecontextid){
        layer.open({
            title: '警告信息',
            ...
            btn1: function(index,layero){
                $.ajax({
                    url: '<%=basePath%>/PageContextEntity/delete.do',
                    type: 'POST',
    ...
    //恢复文章功能
    function restore(pagecontextid){
        layer.open({
            title: '提示信息',
            ...
            btn1: function(index,layero){
                $.ajax({
                    url: '<%=basePath%>/PageContextEntity/restore.do',
                    type: 'POST',
                    data: {pagecontextid: pagecontextid},
                    success: function(data){
                        layer.open({
